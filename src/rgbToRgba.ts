...
const rgbToRgba = (
  ...
  if (isValid('color-rgb', value)) {
    ...
    values = [...values.slice(0, 3).map(item => Math.round(castParam(item))), is('number', a) && +a];

    const [r, g, b] = values;

    return array ? values.filter(value_ => is('number', value_)) : `rgb${is('number', a) ? 'a' : ''}(${r}, ${g}, ${b}${is('number', a) ? `, ${a}` : ''})`;
  }
};
